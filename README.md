...
*   `services/trip-service/` — Микросервис управления поездок и расходов на FastAPI.
    *   `app/models.py` — Модели Trip и Expense.
    *   `app/services/` — Сервисный слой (expense_service).
    *   `app/routers/` — Роутеры (trips, expenses).
