...
class UserPreferenceProfile(BaseModel):
    ...
    preferred_activities: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    trip_type: Mapped[str] = mapped_column(String(50), nullable=False)
    min_safety_threshold: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    profile_type: Mapped[str] = mapped_column(String(50), nullable=False)
    label_destination_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)
