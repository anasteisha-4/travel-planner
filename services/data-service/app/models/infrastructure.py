...
from sqlalchemy import TIMESTAMP, Boolean, Float, ForeignKey, String
...
class DestinationInfrastructure(Base):
    ...
    # Public transport
    has_metro: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="false")
    # Taxi apps (Yandex.Taxi for CIS, Uber global)
    taxi_app_available: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="true")
    ...
    atm_density_score: Mapped[float] = mapped_column(Float, nullable=False, server_default="0.5")
    # Cash economy flag (WB FX.OWN.TOTL.ZS account ownership < 50%)
    cash_economy: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="false")
    # Metadata
    ...
    data_source_details: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
